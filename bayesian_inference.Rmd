...
hist(nc$weight) 
```
Chalo dono dekhte hian 
```{r}
ggplot(data=nc, aes(weight)) + geom_histogram()
```
...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided", hypothesis_prior = c(H1 = 0.1, H2 = 0.9))
...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided",beta_prior=c(75,925),hypothesis_prior = c(H1 = 0.5, H2 = 0.5))
```
